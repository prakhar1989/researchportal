...
class Conf_studentApp extends CI_Controller {
				...
				function load_php()
				{
					 ...
					 //vridhi-changed line below
					 $allow= $this->conference_model->getNoConflast3yr('ankuhfhfhshv');
					 if($allow==True)
					 {
						 echo '
						 <h1>New Conference</h1>
					<p>Please use the form below to enter details of a new conference</p>
				    <form method=POST action="StudentConfApp/insert" ><table class="table table-bordered">
					<thead>
						<tr>
						</tr>
					</thead>
				...
				function insert()
				{
					...
					//vridhi-added line below
					$data['block_num']=0;
					//$data['']=$_POST[''];
					$this->load->model('conference_model');
					$msg=$this->conference_model->insertConference('absdfsf',$data);
					require('showMsg.php');
					$showMsg=new showMsg();
					$showMsg->index($msg);
				}
